...
describe Api::Workflow do
  ...
  context "GET /workflows/id/events" do
    it "returns a workflow object with valid params" do
      ...
      last_response.status.should == 200
      json_response = JSON.parse(last_response.body)
      json_response.should == [{"createdAt"=>Time.now.to_datetime.to_s, "name"=>"WFDecision", "parentId"=>nil, "status"=>"enqueued", "updatedAt"=>Time.now.to_datetime.to_s, "workflowId"=>@wf.id, "id"=>@d1.id, "type"=>"decision", "historyDecisions"=>[], "decider"=>"PaymentDecider", "subjectKlass"=>"PaymentTerm", "subjectId"=>100},
                               {"createdAt"=>Time.now.to_datetime.to_s, "name"=>"WFDecision", "parentId"=>nil, "status"=>"open", "updatedAt"=>Time.now.to_datetime.to_s, "workflowId"=>@wf.id, "id"=>@d2.id, "type"=>"decision", "historyDecisions"=>[{'name' => @d1.name.to_s, 'status' => @d1.reload.status.to_s}], "decider"=>"PaymentDecider", "subjectKlass"=>"PaymentTerm", "subjectId"=>100}]
