...
describe Api::Workflows do
  ...
  context "POST /workflows" do
    ...
    it "returns workflow from database if it already exists" do
      pending "This spec blows up in ci. Need to investigate"
  ...
  context "POST /workflows/id/signal/name" do
    ...
    context "MIGRATING_TYPE" do
      it "returns 201 with 'Delegating Signal' and drops message on queue" do
        Api::Workflows::MIGRATING_TYPE = :crazyworkflow
  ...
  context "GET /workflows/id" do
    it "returns a workflow object with valid params" do
      ...
      response.status.should == 200
      json_response = JSON.parse(response.body)
      json_response.should == {"clientData" => {}, "createdAt"=>FORMAT_TIME.call(Time.now.utc), "decider"=>"PaymentDecider", "mode"=>"blocking", "name"=>"WFType", "parentId"=>nil, "status"=>"open", "subject"=>{"subjectKlass"=>"PaymentTerm", "subjectId"=>"100"}, "updatedAt"=>FORMAT_TIME.call(Time.now.utc), "workflowId"=>nil, "workflowType"=>"WFType", "id"=>@wf.id, "type"=>"workflow", "clientMetadata" => {}}
