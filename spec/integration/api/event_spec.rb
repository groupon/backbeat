...
describe Api::Workflow do
  ...
  context "GET /events/id" do
    it "returns an event object with valid params" do
      ...
      json_response.should == {"createdAt"=>Time.now.to_datetime.to_s, "decider" => "PaymentDecider", "lockedAt"=>nil, "lockedUntil"=>nil, "name"=>"WFDecsion", "parentId"=>nil, "status"=>"enqueued", "updatedAt"=>Time.now.to_datetime.to_s, "workflowId"=>decision.workflow.id, "id"=>decision.id, "type"=>"decision", "pastFlags"=>[], "subjectKlass"=>"PaymentTerm", "subjectId"=>100}
