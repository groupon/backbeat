  ...
  ["/workflows/<%=workflow_id%>/events/<%=event_id%>", "/events/<%=event_id%>"].each do |template|
    context "GET #{template}" do
      it "returns an event object with valid params" do
        ...
        json_response.should == {"clientData"=>{}, "createdAt"=>Time.now.to_datetime.to_s, "decider" => "PaymentDecider", "name"=>"WFDecision", "parentId"=>nil, "status"=>"enqueued", "updatedAt"=>Time.now.to_datetime.to_s, "workflowId"=>@d1.workflow.id, "id"=>@d1.id, "type"=>"decision", "historyDecisions"=>[], "subject"=>{"subjectKlass"=>"PaymentTerm", "subjectId"=>"100"}}
