...
describe V2::Api, v2: true do
  ...
  context "modes" do
    it "completes a workflow with blocking, non_blocking and fire_and_forget nodes" do
      ...
      put "/events/#{activity_nodes[0].id}/status/completed"
      put "/events/#{activity_nodes[1].id}/status/completed"
      ...
      put "/events/#{activity_nodes[2].id}/status/completed"
      ...
      put "/events/#{activity_nodes[3].id}/status/completed"
    ...
    it "completes a workflow with two signals" do
      ...
      WebMock.stub_request(:post, "http://backbeat-client:9000/decision")
        .with(:body => {decision: decision_to_make_2}.to_json)
        .to_return(:status => 200, :body => "", :headers => {})
      ...
      WebMock.stub_request(:post, "http://backbeat-client:9000/activity")
        .with(:body => activity_hash(activity_node_signal_2).to_json)
        .to_return(:status => 200, :body => "", :headers => {})
      V2::Workers::AsyncWorker.drain
      ...
      response = put "/events/#{activity_node_signal_2.id}/status/completed"
