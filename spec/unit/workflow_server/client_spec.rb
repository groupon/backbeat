...
describe WorkflowServer::Client do
  ...
  context "#make_decisions" do
    it "calls the make decision endpoint" do
      ...
      WebMock.stub_request(:post, "http://decisions.com/api/v1/workflows/make_decision").with(:body => {decision: WorkflowServer::Helper::HashKeyTransformations.camelize_keys(decision.serializable_hash)}.to_json, :headers => {'Content-Length'=>'384', 'Content-Type'=>'application/json'} )
      WorkflowServer::Client.make_decision(decision)

      WebMock.should have_requested(:post, "http://decisions.com/api/v1/workflows/make_decision").with(:body => {decision: WorkflowServer::Helper::HashKeyTransformations.camelize_keys(decision.serializable_hash)}.to_json, :headers => {'Content-Length'=>'384', 'Content-Type'=>'application/json'} )
    end
